...
async function boot(){me=await api("/api/me");await render()}
async function nav(v){view=v;await render()}
...
async function render(){
  const page={home:home,gallery:gallery,games:games,pass:pass,feed:feed,leaders:leaders,titles:titles,profile:profile}[view];
  const c=await page();
...
async function post(){let t=$("#postText").value.trim();if(!t)return;await api("/api/posts",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t})});await render()}
async function like(id){await api("/api/like/"+id,{method:"POST"});await render()}
